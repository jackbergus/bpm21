...
public class H_TracesPerspective {
	...
	private void installListeners() {	
		...
		_view.getUpButton().addActionListener(ae -> {
		...
		_view.getDownButton().addActionListener(ae -> {
			if(_view.getTraceList().getSelectedIndex() == -1) {
		...
		_view.getEditTraceButton().addActionListener(ae -> {
		...
		_view.getTracesComboBox().addItemListener(event -> {
		...
		_view.getNextStepButton().addActionListener(ae -> afterLoadingTraces(_view));
	...
	public static void afterLoadingTraces(TracesPerspective _view) {
		assert _view != null;
		if(Constants.getAllTraces_vector().size()==0)  {
			System.err.println("There is no trace defined for the log!\nAt least a trace (even if empty) is required to run the software!");
//				JOptionPane.showMessageDialog(null, "There is no trace defined for the log!\nAt least a trace (even if empty) is required to run the software!", "ATTENTION!", JOptionPane.INFORMATION_MESSAGE, new ImageIcon("images/info_icon.png"));
			...
			//
			Constants.setAlphabetOfTheTraces_vector(new Vector<>());
			...
			for(int i=0;i<_view.getAlphabetListModel().getSize();i++) {
				String string = _view.getAlphabetListModel().getElementAt(i);
		...
		/////////////////// DataAware ////////////////////////
		//////////////////////////////////////////////////////
		Constants.getMenuPerspective().getimportDataAwareListMenuItem().setEnabled(true);
		//////////////////////////////////////////////////////
		//////////////////////////////////////////////////////
	}
