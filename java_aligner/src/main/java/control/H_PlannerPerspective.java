...
public class H_PlannerPerspective {
	...
	private void installListeners() {
		...
		_view.getGeneratePDDLButton().addActionListener(ae -> reduceToPlanningProblem(_view));

		/////// -- END of AAAI2017 -- ////////////////////////////	
		_view.getRunPlannerButton().addActionListener(ae -> {
			dumpOutputs();
		});
		/*
		...
		*/
		_view.getActivitiesComboBox().addItemListener(event -> {
	...
	public static void dumpOutputs() {
		//new ResultsPerspective();
	...
	public static void reduceToPlanningProblem(PlannerPerspective _view) {
...
//			}
			 ...
			 File symba = new File("seq-opt-symba-2");
			 File cc = new File(symba, "Conformance_Checking");
			 File rs = new File(symba, "results");
			 if (!symba.exists()) {
				 symba.mkdirs();
				 cc.mkdirs();
				 rs.mkdirs();
			 } else {
				 if (cc.exists() && rs.exists()) {
					 Utilities.emptyFolder("seq-opt-symba-2/Conformance_Checking");
					 Utilities.emptyFolder("seq-opt-symba-2/results");
				 } else {
					 if (!cc.exists()) cc.mkdirs();
					 else Utilities.emptyFolder("seq-opt-symba-2/Conformance_Checking");
					 if (!rs.exists()) rs.mkdirs();
					 else Utilities.emptyFolder("seq-opt-symba-2/results");
				 }
			 }
			//////////////////////////////////////////////////////////////////////////
			...
			//
			if(Constants.getPlannerPerspective().getCostCheckBox().isSelected()) {
				System.out.println("Setting The Cost Box");
				//UPDATE the ADDING/REMOVAL cost associated to the last activity updated
				String selected_activity_name_for_cost = (String) _view.getActivitiesComboBox().getSelectedItem();
			...
			if(Constants.getPlannerPerspective().getLenght_of_traces_checkBox().isSelected()) {
				length_of_traces_to_check_from = Integer.parseInt(Objects.requireNonNull(Constants.getPlannerPerspective().getLenght_of_traces_ComboBox_FROM().getSelectedItem()).toString());
				length_of_traces_to_check_to = Integer.parseInt(Objects.requireNonNull(Constants.getPlannerPerspective().getLenght_of_traces_ComboBox_TO().getSelectedItem()).toString());
			}
			else {
