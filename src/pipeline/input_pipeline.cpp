...
input_pipeline::convert_trace_labels(const std::string &file, std::unordered_set<std::string> &SigmaAll,
    ...
    if (file.ends_with(".xes")) {
        ...
        data_log = load_xes_with_data(file, ignore_data);
    } else {
...
std::vector<std::vector<std::string>> input_pipeline::print_atomized_traces(const std::string &input_file, const std::string &file_text_and_xes,
                                           std::unordered_set<std::string> &SigmaAll, bool serialize_original_to_xes, bool ignore_data) {
    const auto log = convert_trace_labels(input_file, SigmaAll, serialize_original_to_xes, ignore_data);
    {
...
input_pipeline::decompose_ltlf_for_tiny_graphs(const ltlf &formula,
                                               std::unordered_set<std::string> &SigmaAll,
