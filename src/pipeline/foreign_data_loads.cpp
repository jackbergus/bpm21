...
load_xes_with_data(const std::string &filename, bool ignore_data, bool conservative_for_pddl) {
            ...
            SIBLING_ITERATE2(t, event) {
                ...
                if (tag_name == "string") {
                    if (attribute == concept_name) {
                        ...
                        if (!ignore_data) parsed_event.second[attribute] = value;
                    }
                    ...
                    if (!ignore_data) parsed_event.second[attribute] = std::stod(value);
                    ...
                    if (!ignore_data) parsed_event.second["time"] = (float)parse8601(value);
                }
            }
